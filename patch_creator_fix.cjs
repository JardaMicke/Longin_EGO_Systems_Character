...
// Find the bad block from {activeTab === 'basic' && (  ... all the way to  {activeTab === 'basic' && (
const badRegex = /\{\s*activeTab === 'basic'\s*&&\s*\([\s\S]*?\}\)\s*\}\s*\{\s*activeTab === 'basic'\s*&&\s*\(/;

if (badRegex.test(code)) {
  code = code.replace(badRegex, "{activeTab === 'basic' && (\n              <div className=\"space-y-8 animate-in fade-in slide-in-from-right-8 duration-500\">\n" + galleryContent);
}

// Ensure the outer form doesn't have broken ends
// The issue was we had missing closing tags. `div` inside `activeTab === 'basic'` was closed properly, but let's check `</form>`
const endOfForm = /<\/form>\s*<div className="p-8 border-t border-white\/5/;
if (!endOfForm.test(code)) {
    console.log("Could not find the end of the form!");
}

fs.writeFileSync('components/CharacterCreator.tsx', code);
console.log("CharacterCreator.tsx syntax fixed");
